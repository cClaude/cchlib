...
public class BasicTime implements Serializable, TimeInterface
{
     ...
     */
    public BasicTime( final java.util.Date javadate ) // ---------------------------
    {
        set( javadate );
    }
     ...
     ** @exception BasicTimeException
     */
    public BasicTime( final int hours, final int minutes ) // ----------------------------
            throws BasicTimeException
    {
        set( hours, minutes, 0 );
    }
     ...
     **
     ** @param time
     *            heure sous forme de chaene.
     ** @param formatter
     *            object SimpleDateFormat decrivant le format de la chaene 'time'
     **
     ...
     ** @exception BasicTimeException
     */
    public void set( final int hours, final int minutes, final int seconds ) // -----------------
            throws BasicTimeException
    {
        if( (hours < 0) || (hours > 23) ) {
            ...
            throws BasicDateTimeNegativeValueException
    {
        ...
        final long toMuch = secondsFromMidnight / (24 * 60 * 60);
        secondsFromMidnight -= (toMuch * (24 * 60 * 60));

        final long hours = secondsFromMidnight / (60 * 60);
        secondsFromMidnight -= (hours * (60 * 60));

        final long mins = secondsFromMidnight / 60;
    ...
    /**
     ** Initialise l'objet BasicTime avec une String formatee avec le format interne : SIMPLE_TIME_FORMAT
     ** <P>
     */
     ...
     */
    protected void set( final java.util.Date javaDate ) // --------------------------
    {
    ...
    /**
     ** Surcharge de la methode toString() de la classe Object, permet d'utiliser la classe comme une chaene.
     **
     ...
     */
    @Override
    public boolean equals( final TimeInterface anotherTime ) // ---------------------
    {
     ...
     */
    @Override
    public int compareTo( final TimeInterface anotherTime ) // ----------------------
            throws ClassCastException
    {
        if( anotherTime instanceof BasicTime ) {
            final BasicTime aBasicTime = (BasicTime)anotherTime;
            ...
            final long res = this.longValue() - anotherTime.longValue();
    ...
    /**
     ** @see #compareTo(TimeInterface ) public int compareTo( Object o ) // ---------------------------------------
     *      throws ClassCastException { return compareTo( (TimeInterface)o ); }
     */
     ...
     */
    @Override
    public boolean isBefore( final TimeInterface anotherTime ) // -------------------
    {
        return (this.compareTo( anotherTime ) > 0);
    }
     ...
     *            a BasicTime.
     **
     ** @return true if and only if the instant represented by this BasicTime object is strictly later than the instant
     *         represented by when; false otherwise.
     **
     ** @exception ClassCastException
     */
    @Override
    public boolean isAfter( final TimeInterface anotherTime ) // --------------------
            throws ClassCastException
    {
        return (this.compareTo( anotherTime ) < 0);
    }
     ...
     */
    @Override
    public TimeInterface add( final TimeInterface anotherTime ) // ------------------
            throws BasicDateTimeNegativeValueException
    {
     ...
     */
    @Override
    public TimeInterface sub( final TimeInterface anotherTime ) // ------------------
            throws BasicDateTimeNegativeValueException
    {
        set( this.longValue() - anotherTime.longValue() );
    ...
    public static BasicTime subtract( // --------------------------------------
            final BasicTime basicTime1, final BasicTime basicTime2 ) throws BasicDateTimeNegativeValueException
    {
