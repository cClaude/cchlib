 ...
 *
 */
public class JPanelSearching extends JPanelSearchingWB
{
    ...
    public void initFixComponents( DFToolKit dFToolKit )
    {
        ...
        this.dFToolKit.getJButtonCancel().addMouseListener(
            new MouseAdapter()
            {
                ...
                {
                if( JPanelSearching.this.dFToolKit.getJButtonCancel().isEnabled() ) {
                    JPanelSearching.this.dFToolKit.getJButtonCancel().setEnabled( false );
        ...
        getJTableErrorList().setModel( tableModelErrorList );

        getJProgressBarFiles().setStringPainted( true );
        getJProgressBarOctets().setStringPainted( true );
    }
...
//                slogger.info( "files_:" + files.size() );
    ...
    private void doScanPass2()
    {
        ...
        getJProgressBarFiles().setMaximum( pass1CountFile );
        getJProgressBarOctets().setMaximum( Math.round( pass1BytesCount/1024) );
        duplicateFC.pass2();
...
//                );
            ...
            getJProgressBarFiles().setString( String.format( "%d / %d", pass2CountFile, pass1CountFile ) );
            getJProgressBarOctets().setValue( Math.round( pass2BytesCount/1024) );
            getJProgressBarOctets().setString( String.format( "%d / %d", pass2BytesCount, pass1BytesCount ) );
        }
    }
            ...
            )
    {
        ...
        getJProgressBarOctets().setIndeterminate( true );
