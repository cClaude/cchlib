 ...
 ** @version 3.01.042
 */
public class MD5FileCollectionBridgeImpl implements MD5FileCollection {
    ...
    /**
     ** Retourne un ensemble, non modifiable, d'objet {@link File} représentant
     * le le nom complet de l'ensemble des dossiers connus de cette collection.
     **
    ...
    public Set<File> getFolderFiles() // --------------------------------------
    {
        if( this._transient_dbFoldersFiles == null ) {
            this._transient_dbFoldersFiles = toSetOfFile( this.masterMD5Collection
                    .getFolderFilenames() );
        }
    ...
    public Map<MD5TreeEntry, ? extends Set<File>> getEntryFiles() // -----------
    {
        if( this._transient_dbMessageDigestFiles == null ) {
            this._transient_dbMessageDigestFiles = toMapOfFile( this.masterMD5Collection
                    .getEntryFilenames() );
        }
    ...
    /**
     ** Transforme un ensemble de nom de fichiers en un ensemble trié d'objets
     * File
     */
    ...
    {
        final SortedMap<MD5TreeEntry, SortedSet<File>> map = new TreeMap<MD5TreeEntry, SortedSet<File>>();

        for( Map.Entry<MD5TreeEntry, ? extends Set<String>> entry : mapsrc
                .entrySet() ) {
            map.put( entry.getKey(), toSetOfFile( entry.getValue() ) );
        }
