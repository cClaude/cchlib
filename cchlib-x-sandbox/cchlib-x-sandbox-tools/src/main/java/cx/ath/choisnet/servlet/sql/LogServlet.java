...
/**
** <P><B>Servlet :</B> Ajoute une entre dans la table des logs.</P>
**
**
    ...
    throws ServletException
{
 ...
 try {
    this.dataSource = DataSourceFactory.buildMySQLDataSource( "127.0.0.1", "tools", "tomcat", "tomcat" );
    }
 catch( Exception e ) {
        ...
        java.io.IOException
{
 ...
 final String   col_LogDate         = MySQLTools.parseFieldValue(
                                        getParameter( request, "DATE" , "LOGDATE"     , "LogDate"         ),
                                        24
                                        );
 final String   col_AppName         = MySQLTools.parseFieldValue(
                                        getParameter( request, "AN"   , "APPNAME"     , "AppName"         ),
                                        64
                                        );
 final String   col_Message         = MySQLTools.parseFieldValue(
                                        getParameter( request, "MSG"  , "MESSAGE"     , "Message"         ),
                                        255
                                        );
 ...
 final String query
    = "INSERT INTO `Logger` (`AutoLogDate`,`AutoIpAddress`,`AutoComputerName`, "
        + "`ComputerName`, `LogDate`, `AppName`, `Message`) "
        + "VALUES ( NOW(),'" + col_AutoIpAddress + "','" + col_AutoComputerName + "', "
        + "'" + col_ComputerName + "', '" + col_LogDate + "', "
        + "'" + col_AppName + "', '" + col_Message + "');";
