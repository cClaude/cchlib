...
public void init() throws javax.servlet.ServletException // ---------------
{
 ...
 // DUMP_DIRECTORY
 //
 this.dumpDirectory = new File( ssc.getRequiredParameter( DUMP_DIRECTORY ) );

 if( ! dumpDirectory.isDirectory() ) {
    throw new javax.servlet.ServletException(
        "parameter '" + DUMP_DIRECTORY + "' not valid : '" + dumpDirectory + "'"
        );
    }
}
        ...
        javax.servlet.ServletException
{
 ...
 if( filename != null ) {
    outputFile = new File(
                        dumpDirectory,
                        getCurrentDateForFileName() + "_" + filename + ".sql"
                        );
    }
 else {
    outputFile = null;
 ...
 if( "FILE".equals( outputMode ) ) {
    ...
    actionCreateDumpFile( request, response.getOutputStream(), outputFile );
...
/**
** Retourne une cha�ne formatt�e, compatible avec un nom de fichier contenant
** les date et heure courantes.
**
