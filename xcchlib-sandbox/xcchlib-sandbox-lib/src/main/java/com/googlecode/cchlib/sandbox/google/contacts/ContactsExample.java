...
public class ContactsExample {
 ...
 public ContactsExample(final ContactsExampleParameters parameters)
   ...
   final String url = parameters.getBaseUrl()
  ...
  */
 private void deleteEntry(final ContactsExampleParameters parameters)
 ...
 public void updateEntry(final ContactsExampleParameters parameters)
   ...
   if (parameters.isGroupFeed()) {
     ...
     // update fields
     final List<ExtendedProperty> extendedProperties =
  ...
  */
 private ContactEntry getContactInternal(final String id)
     throws IOException, ServiceException {
  ...
  */
 private ContactGroupEntry getGroupInternal(final String id)
     throws IOException, ServiceException {
   return service.getEntry(
       new URL(id.replace("/base/", "/" + projection + "/")),
 ...
 private static void printContact(final ContactEntry contact) {
   ...
   final Link photoLink = contact.getLink(
  ...
  */
 private static void processScript(final ContactsExample example, final ContactsExampleParameters parameters)
         throws IOException, ServiceException
 {
  ...
  */
 private void queryEntries(final ContactsExampleParameters parameters)
     throws IOException, ServiceException {
   ...
   try {
     if (parameters.isGroupFeed()) {
       final ContactGroupFeed groupFeed = service.query(
       ...
       final ContactFeed resultFeed = service.query(myQuery, ContactFeed.class);
 ...
 {
   if (parameters.isGroupFeed()) {
     final ContactGroupFeed groupFeed =  service.getFeed(feedUrl, ContactGroupFeed.class);
     ...
     final ContactFeed resultFeed = service.getFeed(feedUrl, ContactFeed.class);
     // Print the results
     ...
     for (final ContactEntry entry : resultFeed.getEntries()) {
       ...
       final Link photoLink = entry.getLink(
           "http://schemas.google.com/contacts/2008/rel#photo", "image/*");
  ...
  */
 private void addEntry(final ContactsExampleParameters parameters)
     throws IOException, ServiceException {
   if (parameters.isGroupFeed()) {
     final ContactGroupEntry addedGroup =
         service.insert(feedUrl, buildGroup(parameters));
     printGroup(addedGroup);
     lastAddedId = addedGroup.getId();
   } else {
     final ContactEntry addedContact =
